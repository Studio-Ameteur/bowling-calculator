    ...
    def _build_ui(self):
        ...
        for i in range(11):
            btn = tk.Button(top, text=str(i), width=3, relief="groove",
                            bg="#2a2a2a", fg=WHITE, font=("Arial", 10), activebackground="#444", activeforeground=WHITE,
...
class OverlayWindow(tk.Toplevel):
    ...
    def _draw_table(self):
        ...
        header = tk.Frame(self.canvas_frame, bg=BG_HEAD)
        header.pack(fill="x", pady=(0, 1))

        tk.Label(header, text="", bg=BG_HEAD, width=int(NAME_W/7)).pack(side="left")
        for f in last_frames:
            tk.Label(header, text=str(f + 1), bg=BG_HEAD, fg=FG_GRAY,
                     font=("Arial", 9, "bold"), width=int(CELL_W/7),
                     anchor="center").pack(side="left", padx=1)
        tk.Label(header, text="MAX", bg=BG_HEAD, fg=FG_GRAY,
                 font=("Arial", 9, "bold"), width=int(MAX_W/7)).pack(side="left", padx=1)
        ...
        for idx, s in enumerate(shown):
            ...
            row = tk.Frame(self.canvas_frame, bg=row_bg)
            row.pack(fill="x", pady=1)

            tk.Label(row, text=s.get_name(), bg=row_bg, fg=FG_WHITE,
                     font=("Arial", 10, "bold"), width=int(NAME_W/7),
            ...
            for f in last_frames:
                ...
                cell = tk.Frame(row, bg=cell_bg, width=CELL_W, height=CELL_H)
                cell.pack(side="left", padx=1, pady=1)
                cell.pack_propagate(False)

                top_part = tk.Frame(cell, bg=cell_bg)
                top_part.pack(fill="x")

                roll_text = " ".join(d) if d else ""
                tk.Label(top_part, text=roll_text, bg=cell_bg, fg=FG_WHITE,
                         font=("Arial", 9, "bold"), anchor="center").pack(expand=True)

                if f < len(cumulative_all):
                    sc_text = str(cumulative_all[f])
                    tk.Label(cell, text=sc_text, bg=cell_bg, fg=FG_YELLOW,
                             font=("Arial", 8), anchor="center").pack()

            max_val = s.get_max()
            diff_text = ""
            if len(shown) > 1 and diff != 0:
                diff_text = f"{diff:+d} " if diff < 0 else f"+{diff} "

            max_frame_widget = tk.Frame(row, bg=row_bg, width=MAX_W)
            max_frame_widget.pack(side="left", padx=1)
            max_frame_widget.pack_propagate(False)

            if diff_text:
                tk.Label(max_frame_widget, text=diff_text, bg=row_bg,
                         fg="#ff6666" if diff < 0 else "#66ff66",
                         font=("Arial", 8, "bold")).pack(side="left")
            tk.Label(max_frame_widget, text=str(max_val), bg=row_bg, fg=FG_YELLOW,
