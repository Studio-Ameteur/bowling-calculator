...
class BowlingSeries:
    def __init__(self, parent, app, idx):
        ...
        self.widget = tk.Frame(parent, bg=GREEN, bd=2, relief="solid")
        self._build()

    def _build(self):
        header = tk.Frame(self.widget, bg=GREEN)
        header.pack(fill="x")

        tk.Button(header, text="×", bg=GREEN, fg=WHITE,
        ...
        for f in range(10):
            ...
            tk.Label(col, text=str(f+1), bg=GRAY_BG,
        ...
        self.total_lbl = tk.Label(score_col, text="", bg=YELLOW_BG,
                                  font=("Arial", 10, "bold"), width=6)
        ...
        self.max_lbl = tk.Label(max_col, text="300", bg=GRAY_BG,
                                font=("Arial", 10, "bold"), width=6)
    ...
    def refresh(self):
        ...
        for f in range(10):
            ...
            labels[0].config(text=d[0])
            labels[1].config(text=d[1])
            if labels[2] is not None:
                labels[2].config(text=d[2])
...
class BowlingApp:
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="ДОБАВИТЬ СЕРИЮ »", bg=GRAY_BG,
                  font=("Arial", 10, "bold"), relief="groove",
                  command=self.add_series).pack(side="left", padx=8)

        tk.Button(top, text="Оверлей", bg=GRAY_BG,
                  font=("Arial", 10, "bold"), relief="groove",
                  command=self.show_overlay).pack(side="left", padx=4)

        self.undo_btn = tk.Button(top, text="← Отмена", bg="#c0392b", fg=WHITE,
                  font=("Arial", 10, "bold"), relief="groove",
                  command=self.undo_last)
        self.undo_btn.pack(side="left", padx=8)

        self.container = tk.Frame(self.root, bg=GRAY_BG)
    ...
    def update_pin_buttons(self):
        if not self.active_series or self.active_series.current_frame >= 10:
            for btn in self.pin_buttons:
                btn.config(state="disabled", bg="#bbb")
            return
        allowed = self.active_series.get_allowed_pins()
...
class OverlayWindow(tk.Toplevel):
    ...
    def _build(self):
        hdr = tk.Frame(self, bg="#4a7c4e", pady=6)
        hdr.pack(fill="x")
        tk.Label(hdr, text="БОУЛИНГ — РЕЗУЛЬТАТЫ", bg="#4a7c4e", fg=WHITE,
                 font=("Arial", 11, "bold"), padx=12).pack(side="left")

        self.body = tk.Frame(self, bg="#1a1a1a")
        self.body.pack(fill="both", expand=True, padx=10, pady=8)

        footer = tk.Frame(self, bg="#1a1a1a", pady=6)
        footer.pack(fill="x", padx=10)
        tk.Label(footer, text="Лучший результат:", bg="#1a1a1a", fg="#888",
                 font=("Arial", 10)).pack(side="left")
        self.best_lbl = tk.Label(footer, text="—", bg="#1a1a1a", fg="#f0c040",
                                 font=("Arial", 13, "bold"))
        self.best_lbl.pack(side="left", padx=6)

    def refresh(self):
        for w in self.body.winfo_children():
            w.destroy()

        tk.Label(self.body, text="ПОСЛЕДНИЕ 3 СЕРИИ", bg="#1a1a1a", fg="#888",
                 font=("Arial", 8, "bold")).pack(anchor="w", pady=(0, 6))

        recent = self.app.series_list[-3:]
        for i, s in enumerate(recent):
            bg = "#222" if i % 2 == 0 else "#1e1e1e"
            ...
            tk.Label(row, text=str(s.get_total()), bg=bg, fg="#f0c040",
                     font=("Arial", 14, "bold"), width=5).pack(side="left")
            tk.Label(row, text=f"MAX: {s.get_max()}", bg=bg, fg="#5dde5d",
                     font=("Arial", 10), width=9).pack(side="left")

        if not recent:
            tk.Label(self.body, text="Нет данных", bg="#1a1a1a", fg="#888",
                     font=("Arial", 10)).pack(pady=10)

        tk.Frame(self.body, bg="#333", height=1).pack(fill="x", pady=8)

        totals = [s.get_total() for s in self.app.series_list if s.get_rolls_flat()]
        best = max(totals) if totals else None
        self.best_lbl.config(text=str(best) if best is not None else "—")
